...
export default function TimesheetCalendar() {
  ...
  // ── NEW: day-mode state ──────────────────────────────────────────────────────
  const [dayMode, setDayMode] = useState("");          // "" | "leave" | "holiday"
  ...
  const getCellStatus = (date) => {
    ...
    // API holiday (from holidayList) — block
    const holiday = getHolidayForDay(date);
    if (holiday) return "holiday";

    // API leave (from leaveList) — block only if no manual hours saved
    ...
    // Manual timesheet entries (leaveType saved by user)
    if (entry?.leaveType && !entry?.hours) return "leave";   // show leave color
  ...
  const handleCellClick = (date) => {
    ...
    if (!canEdit) {
      ToastError("Previous month timesheets are locked for editing");
      return;
    }

    const status = getCellStatus(date);

    // Block API-sourced leaves (leaveList has an entry for this date)
    ...
    if (apiHoliday) return;                          // real API holiday → block
    if (apiLeave && !apiLeave.approverReason) return; // real API leave → block

    // Everything else (including manually saved leaveType entries) → allow edit
    setSelDate(date);
    const entry = entries[date.format("YYYY-MM-DD")];
    ...
    if (savedLeave.startsWith("Holiday:")) {
      setDayMode("holiday");
      setManualHolidayName(savedLeave.replace("Holiday:", "").trim());
      setLeaveType("");
    } else if (savedLeave) {
      setDayMode("leave");
      setLeaveType(savedLeave);
  ...
  const saveEntry = () => {
    ...
    // ── Build resolvedLeave from dayMode ──────────────────────────────────────
    ...
    upd[selDate.format("YYYY-MM-DD")] = {
      task: validTasks.length ? JSON.stringify(validTasks) : "",
      hours: totalHrs,
      leaveType: !totalHrs ? resolvedLeave : null,   // ← uses resolvedLeave
    };
              ...
              {weeks.map((week, wi) => (
                <tr key={wi}>
                  {week.map(day => {
                    ...
                    // holiday cells are now clickable so user can mark manual holiday
              ...
              {totalHours === 0 && (
                <div style={{ marginBottom: 14 }}>
                    ...
                    {[
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: `1.5px solid ${dayMode === val ? "var(--primary)" : "var(--border)"}`,
                          background: dayMode === val ? "var(--primary-ghost)" : "var(--bg)",
                          color: dayMode === val ? "var(--primary)" : "var(--text-secondary)",
                          boxShadow: dayMode === val ? "0 0 0 3px var(--primary-ghost)" : "none",
                        }}
                      >
                  ...
                  {dayMode === "leave" && (
                    <>
                      <select
                        ...
                        style={{
                          borderColor: !leaveType ? "var(--coral)" : undefined,
                          boxShadow: !leaveType ? "0 0 0 3px rgba(239,68,68,0.1)" : undefined,
                        }}
                      >
                  ...
                  {dayMode === "holiday" && (
                    <>
                      <input
                        className={`fc-input${!manualHolidayName.trim() ? " fc-input-error" : ""}`}
                        placeholder="e.g. Diwali, Christmas, Pongal…"
                        value={manualHolidayName}
                        onChange={e => setManualHolidayName(e.target.value)}
                        style={{ marginBottom: !manualHolidayName.trim() ? 0 : undefined }}
                      />
                ...
                {copyOpt === "custom" && (
                  <div style={{ marginTop: 10 }}>
                    ...
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                      ...
                      {allDays.filter(d => {
                        if (d.month() !== currentMonth.month()) return false;
                        if (d.isAfter(today, "day")) return false;
                        if (getHolidayForDay(d)) return false;                          // hide API holidays
                        if (getLeaveForDay(d) && !getLeaveForDay(d)?.approverReason) return false; // hide API leaves
                        return true;
                      }).map(d => {
                        const iso = d.format("YYYY-MM-DD");
