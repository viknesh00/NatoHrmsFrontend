...
import { CalendarCheck2, Check, RotateCcw } from "lucide-react";
...
export default function Attendance() {
  ...
  useEffect(() => {
    ...
    else getMonthlyLog(dayjs().subtract(1,"month").format("YYYY-MM-DD"), dayjs().format("YYYY-MM-DD"));
  }, [logType]);
  ...
  const getMonthlyLog = (from, to) => {
    ...
    postRequest("Attendance/GetMonthlyAttendance", { fromDate: from, toDate: to })
      .then(res => {
        if (res.data) setFilteredData(res.data.map(d => ({
          ...d,
  ...
  const handleFilter = () => {
    if (logType === "daily") { getDailyLog(singleDate); ToastSuccess(`Filtered for ${dayjs(singleDate).format("DD-MM-YYYY")}`); }
    else {
      if (dayjs(toDate).isBefore(dayjs(fromDate))) { ToastError("To date must be after From date"); return; }
  ...
  const handleReset = () => {
    ...
    if (logType === "daily") { setSingleDate(today); getDailyLog(today); }
    else { setFromDate(prev); setToDate(today); getMonthlyLog(prev, today); }
  ...
  const columnsMonthly = [
    {
      ...
      renderCell: (row) => <span style={{ color: (row.status === "WO" || row.status === "Holiday") ? "var(--coral)" : "var(--text-primary)" }}>{row.attendanceDate}</span>,
    },
    ...
    { field: "clockInIp", label: "Clock In IP" },      // ← missing
    { field: "clockOutIp", label: "Clock Out IP" },     // ← missing
    { field: "clockInLocation", label: "Clock In Location" },
    { field: "clockOutLocation", label: "Clock Out Location" }, // ← missing
  ];

  const inputStyle = {
    padding: "7px 11px", borderRadius: 8, border: "1.5px solid var(--border)",
  ...
  const filterBar = (
    <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
      {logType === "daily" ? (
        <input type="date" style={inputStyle} value={singleDate} onChange={e => setSingleDate(e.target.value)} />
      ) : (
        <>
      ...
      <button className="btn btn-primary btn-sm" onClick={handleFilter}><Check size={14} /> Apply</button>
